...
class BaseAlgorithm(ABC):
    ...
    def __init__(
        ...
        self.seed = seed
        self.action_noise_p1: Optional[ActionNoise] = None
        self.action_noise_p2: Optional[ActionNoise] = None
        self.start_time = 0.0
        ...
        self.ep_p1_info_buffer = None  # type: Optional[deque]
        self.ep_p2_info_buffer = None  # type: Optional[deque]
        self.ep_p1_success_buffer = None  # type: Optional[deque]
        self.ep_p2_success_buffer = None  # type: Optional[deque]
        ...
        if env is not None:
            ...
            env = self._wrap_env(env, self.verbose, monitor_wrapper=False)
    ...
    def _setup_learn(
        ...
        if self.ep_p1_info_buffer is None or reset_num_timesteps:
            # Initialize buffers if they don't exist, or reinitialize if resetting counters
            self.ep_p1_info_buffer = deque(maxlen=self._stats_window_size)
            self.ep_p2_info_buffer = deque(maxlen=self._stats_window_size)
            self.ep_p1_success_buffer = deque(maxlen=self._stats_window_size)
            self.ep_p2_success_buffer = deque(maxlen=self._stats_window_size)

        if self.action_noise_p1 is not None:
            self.action_noise_p1.reset()
            self.action_noise_p2.reset()
    ...
    def _update_info_buffer(self, infos: List[Dict[str, Any]], dones: Optional[np.ndarray] = None) -> None:
        ...
        assert self.ep_p1_success_buffer is not None
        assert self.ep_p2_success_buffer is not None
        ...
        for idx, info in enumerate(infos):
            ...
            if maybe_ep_info is not None:
                self.ep_p1_info_buffer.extend([maybe_ep_info])
                self.ep_p2_info_buffer.extend([maybe_ep_info])
            if maybe_is_success is not None and dones[idx]:
                self.ep_p1_success_buffer.append(maybe_is_success)
                self.ep_p2_success_buffer.append(maybe_is_success)
    ...
    def load(  # noqa: C901
        ...
        **kwargs,
        ...
        if env is not None:
            ...
            env = cls._wrap_env(env, data["verbose"], monitor_wrapper=False)
